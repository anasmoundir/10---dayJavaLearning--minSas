...
//    public static void retournerLivre(Scanner scanner, String titre) {
//        try {
//            Livre livre = BookDao.rechercherLivresParTitre(titre);
//            if (livre != null) {
//                String rechercheReservationQuery = "SELECT id FROM reservation WHERE livre_copy_id IN (SELECT id FROM livrecopy WHERE livre_id = ?)";
//                PreparedStatement rechercheReservationStatement = connection.prepareStatement(rechercheReservationQuery);
//                rechercheReservationStatement.setInt(1, livre.getId_livre());
//                ResultSet reservationResultSet = rechercheReservationStatement.executeQuery();
//
//                if (reservationResultSet.next()) {
//                    int reservationId = reservationResultSet.getInt("id");
//
//
//                    String retourQuery = "UPDATE reservation SET date_retour = ? WHERE id = ?";
//                    PreparedStatement retourStatement = connection.prepareStatement(retourQuery);
//                    LocalDate dateRetour = LocalDate.from(LocalDateTime.now());
//                    retourStatement.setDate(1, Date.valueOf(dateRetour));
//                    retourStatement.setInt(2, reservationId);
//                    retourStatement.executeUpdate();
//
//                    updateLivreCopyState(livre.getId_livre(), 0);
//
//                    System.out.println("Livre retourné avec succès !");
//                } else {
//                    System.out.println("Aucune réservation trouvée pour ce livre.");
//                }
//            } else {
//                System.out.println("Livre introuvable.");
//            }
...
//    }
    ...
    public static void afficherLivresEmpruntes() {
        try {
            Connection connection = Database.getConnection();
            String query = "SELECT l.titre, r.date_reservation, r.date_limit " +
                    "FROM reservation r " +
                    "INNER JOIN livrecopy lc ON r.livre_copy_id = lc.id " +
                    "INNER JOIN livre l ON lc.livre_id = l.id " +
                    "WHERE lc.etat = 1";
            ...
            while (resultSet.next()) {
                ...
                LocalDate dateReservation = resultSet.getDate("date_reservation").toLocalDate();
                LocalDate dateLimite = resultSet.getDate("date_limit").toLocalDate();
